...
public class TzJsonController {
    ...
    @Autowired
    private YolohoJsonService yolohoJsonService;
     ...
     */
    @ResponseBody
    @RequestMapping(value = "/tzRequest",method = {RequestMethod.POST})
    public ResponseResult TzJsonRequest(@Valid @RequestBody TzBidRequest request) throws IOException {
        log.info("=========下游请求参数"+JSONObject.toJSON(request));
        TzBidResponse bidResponse = taskService.ckJsonRequest(request);
