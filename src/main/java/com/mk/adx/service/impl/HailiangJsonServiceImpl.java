...
public class HailiangJsonServiceImpl implements HailiangJsonService {
    ...
    @SneakyThrows
    @Override
    public TzBidResponse getHailiangDataByJson(TzBidRequest request) {
        ...
        if (null!=request.getImp()) {
            for (int i=0;i<request.getImp().size();i++){
                ...
                imp.setId(request.getImp().get(i).getId());//必填 标识⼀个Imp对象的唯⼀ID
                imp.setSlot_id(request.getAdv().getTag_id());//必填，当前Imp所对应的⼴告位ID（嗨量提供）
                String ad_slot_type = request.getImp().get(i).getAd_slot_type();//广告位类型
            ...
            TzMacros tzMacros = new TzMacros();
                    for (int i = 0; i < data.size(); i++) {
                        ...
                        for (int j = 0; j < bid.size(); j++) {
                            tb.setAdid(id);
                            tb.setId(bid.getJSONObject(j).getString("id"));
                            ...
                            if ("1".equals(request.getImp().get(i).getAd_slot_type())) {
                                ...
                                image.setW(jsonObject.getInteger("w"));
                                image.setH(jsonObject.getInteger("h"));
                                list.add(image);
                                tb.setImages(list);
                            }
