...
public class YdzxJsonServiceImpl implements YdzxJsonService {
    ...
    @SneakyThrows
    @Override
    public TzBidResponse getYdzxDataByJson(TzBidRequest request) {
        ...
        if (null!=request.getImp()){
            for (int i=0;i<request.getImp().size();i++){
                ...
                if(null == request.getImp().get(i).getBidfloor()){
                    ...
                    float bidfloor = 0l;
                    bidfloor = request.getImp().get(i).getBidfloor();
                    int bf = (int) bidfloor;  //底价,单位：分/CPM
                    imp.setBidfloor(bf);   //可变
                }
                impList.add(imp);
            }
        }
        ...
        String url = "http://dsp2.yidianzixun.com/bid?adxType=tianzhuo_1020&rtbVersion=3";//测试url
         ...
         if (null != response){
            ...
            for (int i=0;i<seatids.size();i++){
                ...
                for (int j=0;j<bids.size();j++){
                    TzBid tb = new TzBid();
                    tb.setId(bids.getJSONObject(j).getString("id"));//DSP 产生的本次BID 的ID，用于日志与跟踪

                    if (null!=bids.getJSONObject(j).getInteger("price")){
                        tb.setPrice(bids.getJSONObject(j).getInteger("price"));//CPM出价，单位分
                    }

                    String win = bids.getJSONObject(j).getString("nurl").replace("%%WIN_PRICE%%", "0");
                    String nurl = win + "," + id + "," + request.getImp().get(0).getTagid() + "," + request.getDevice().getIp() + "," + name + "," + request.getApp().getName();
                    tb.setNurl("http://adx-bid.tianzhuobj.com/ad-core-master/api/feedback/winPrice?price=" + nurl + "&winNotice=%%WIN_PRICE%%");//Win Notice Url
                    ...
                    tb.setAdid(bids.getJSONObject(j).getString("adid"));//广告ID
                    tb.setCrid(bids.getJSONObject(j).getString("crid"));//创意ID
                    tb.setH(bids.getJSONObject(j).getInteger("h"));//素材高，单位像素
                    tb.setW(bids.getJSONObject(j).getInteger("w"));//素材宽，单位像素
                    ...
                    if(null != bids.getJSONObject(j).getJSONObject("appInfo")){
                        ...
                        tzBidApps.setApp_name(appInfo.getString("appName"));
                        tb.setApp(tzBidApps);
                    }
