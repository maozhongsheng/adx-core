...
import com.mk.adx.entity.json.request.simeng.SmBidRequest;
import com.mk.adx.entity.json.request.simeng.SmDevice;
import com.mk.adx.entity.json.request.simeng.SmNetWork;
import com.mk.adx.entity.json.request.tz.TzBidRequest;
import com.mk.adx.entity.json.response.tz.*;
import com.mk.adx.service.SmJsonService;
 ...
 */
@Slf4j
@Service("smJsonService")
public class SmJsonServiceImpl implements SmJsonService {

    private static final String name = "sm";
    private static final String source = "思盟";
    /**
     * @Author mzs
     * @Description 思盟
     * @Date 2021/6/28 9:48
    ...
    @SneakyThrows
    @Override
    public TzBidResponse getSmDataByJson(TzBidRequest request) {
        SmBidRequest bidRequest = new SmBidRequest();
        SmDevice device = new SmDevice();
        SmNetWork netWork = new SmNetWork();
        ...
        if(0f == request.getDevice().getGeo().getLat()){
            netWork.setLat(0);
        }else{
            netWork.setLat(request.getDevice().getGeo().getLat());
        }

        if(0f == request.getDevice().getGeo().getLon()){
        ...
        bidRequest.setChannelId(request.getAdv().getTag_id());  //  D1000016

        //总返回
        TzBidResponse bidResponse = new TzBidResponse();
        String content = JSONObject.toJSONString(bidRequest);
        log.info("请求思盟广告参数"+JSONObject.parseObject(content));
        String url = "http://api.simengadx.com/api/ads";   //http://8.134.52.3/ads/bid
        ...
        log.info("请求上游思盟广告花费时间："+
                (((tempTime/86400000)>0)?((tempTime/86400000)+"d"):"")+
                ((((tempTime/86400000)>0)||((tempTime%86400000/3600000)>0))?((tempTime%86400000/3600000)+"h"):(""))+
                ((((tempTime/3600000)>0)||((tempTime%3600000/60000)>0))?((tempTime%3600000/60000)+"m"):(""))+
                ((((tempTime/60000)>0)||((tempTime%60000/1000)>0))?((tempTime%60000/1000)+"s"):(""))+
                ((tempTime%1000)+"ms"));
        log.info("思盟广告返回参数"+JSONObject.parseObject(response));
        JSONObject jo = JSONObject.parseObject(response);
        if (null!=jo){
            if ("0".equals(jo.getString("errorCode"))) {
                ...
                for (int i = 0; i < imp.size(); i++) {
                    ...
                    for (int meta = 0; meta < metaGroup.size(); meta++) {
                        ...
                        if ("1".equals(request.getImp().get(0).getAd_slot_type())) {
                            ...
                            JSONArray SmImages = metaGroup.getJSONObject(meta).getJSONArray("imageUrl");
                            ArrayList<TzImage> images = new ArrayList<>();
                            if (null != SmImages) {
                                for (int im = 0; im < SmImages.size(); im++) {
                                    TzImage tzImage = new TzImage();
                                    tzImage.setUrl(SmImages.getString(im));
                            ...
                            JSONArray SmDes =  metaGroup.getJSONObject(meta).getJSONArray("descs");
                            if (null != SmDes) {
                                tzNative.setDesc(SmDes.get(0).toString());
                            }

                            JSONArray SmIcon = metaGroup.getJSONObject(meta).getJSONArray("iconUrls");
                            if(null != SmIcon){
                                tzIcon.setUrl(SmIcon.getString(0));
                            ...
                            ArrayList<TzImage> images = new ArrayList<>();
                            if (null != SmImages) {
                                for (int cqim = 0; cqim < SmImages.size(); cqim++) {
                                    TzImage tzImage = new TzImage();
                                    tzImage.setUrl(SmImages.getString(cqim));
                                    tzImage.setH(metaGroup.getJSONObject(meta).getInteger("materialHeight"));
                                    tzImage.setW(metaGroup.getJSONObject(meta).getInteger("materialWidth"));
                                    images.add(tzImage);
                                }
                            ...
                            JSONArray SmDes =  metaGroup.getJSONObject(meta).getJSONArray("descs");
                            if (null != SmDes) {
                                tb.setDesc(SmDes.get(0).toString());
                            }
                            JSONArray SmIcon = metaGroup.getJSONObject(meta).getJSONArray("iconUrls");
                            if(null != SmIcon){
                                tb.setAic(SmIcon.get(0).toString());
                            }
                        }
...
//                }
                ...
                log.info("思盟广告总返回"+JSONObject.toJSONString(bidResponse));
            }
        }
