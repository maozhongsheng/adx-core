...
import com.mk.adx.service.ZxsjJsonService;
 ...
 */
@Slf4j
@Service("zxsjJsonService")
public class ZxsjJsonServiceImpl implements ZxsjJsonService {

    private static final String name = "自旋视界";

    private static final String source = "自旋视界";

    @SneakyThrows
    @Override
    public TzBidResponse getZxsjDataByJson(TzBidRequest request) {
//        ZxsjBidRequest bidRequest = new ZxsjBidRequest();
//        bidRequest.setId(request.getId());
//        bidRequest.setSlot_id(request.getAdv().getTag_id());
       ...
       // device.setDpi();
        ...
        log.info(request.getImp().get(0).getTagid() + "请求自旋视界广告参数"+JSONObject.parseObject(content));
        ...
        log.info(request.getImp().get(0).getTagid()+":请求上游自旋视界广告花费时间："+
                (((tempTime/86400000)>0)?((tempTime/86400000)+"d"):"")+
                ((((tempTime/86400000)>0)||((tempTime%86400000/3600000)>0))?((tempTime%86400000/3600000)+"h"):(""))+
                ((((tempTime/3600000)>0)||((tempTime%3600000/60000)>0))?((tempTime%3600000/60000)+"m"):(""))+
                ((((tempTime/60000)>0)||((tempTime%60000/1000)>0))?((tempTime%60000/1000)+"s"):(""))+
                ((tempTime%1000)+"ms"));
        log.info(request.getImp().get(0).getTagid()+":自旋视界广告返回参数"+JSONObject.parseObject(response));
        ...
        if (null != response && 0 == JSONObject.parseObject(response).getInteger("error_code")) {
            ...
            log.info(request.getImp().get(0).getTagid()+":自旋视界总返回数据"+JSONObject.toJSONString(bidResponse));
        }
        return bidResponse;
    }
}
