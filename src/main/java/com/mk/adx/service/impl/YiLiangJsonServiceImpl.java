...
public class YiLiangJsonServiceImpl implements YiLiangJsonService {
    ...
    @SneakyThrows
    @Override
    public TzBidResponse getYiLiangDataByJson(TzBidRequest request) {
           ...
           // String msg = jo.getString("msg");//广告响应说明
            ...
            for (int i = 0; i < imp.size(); i++) {
                ...
                if(5 == imp.getJSONObject(i).getInteger("adType")){
                    ...
                    tzLogo.setUrl(imp.getJSONObject(i).getString("adLogo"));
                    ...
                    Integer adType = imp.getJSONObject(i).getInteger("adType");
                    if(2 == adType){
                        tb.setAd_type(5);//开屏-广告素材类型
                    }else if(3 == adType){
                        tb.setAd_type(0);//banner-广告素材类型
                    }else if(4 == adType){
                        tb.setAd_type(3);//插屏-广告素材类型
                    }
                    JSONObject ylImages =  imp.getJSONObject(i).getJSONObject("image");
                    ArrayList<TzImage> images = new ArrayList<>();
                    if (null != ylImages) {
